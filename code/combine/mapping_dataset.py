...
localHost = '/content/drive/MyDrive/largeProject/code/combine/template_generate'
...
# with open('/content/drive/MyDrive/largeProject/code/combine/saved_dictionary.pkl', 'rb') as f:
...
def mappingOneKPI(loaded_dict,timeFind, view = "", company="", kpi = "",dict_index={"indexNow":None,
                                                                       "indexBefore":None,
    ...
    template = postProcessOutput(template)
    ##############
...
def mappingGroupMonthOverall(loaded_dict,timeFind,view="",company="",groupKPI="",index = None):
    ##for test:
    if view == "":
        view = random.choice(["month","quarter","year"])
    if company == "":
    ...
    #get template
    template = postProcessOutput(b_monthView.genGroupMonthOverall(kpiList,evaluationList,view,index))
...
def mappingGroupMonthDetail(loaded_dict,timeFind,view="",company="",groupKPI="",index = None):
    ##for test:
    if view == "":
        view = random.choice(["month","quarter","year"])
    if company == "":
    ...
    if len(kpiList) == 1:
        dataset = loaded_dict[timeFind][view][company][kpiList[0]]
        if dataset['KPI MẸ'] == groupKPI:
            return f'cụm chỉ tiêu "{groupKPI}" này được coi là 1 chỉ tiêu (vì nó không có chỉ tiêu con). ' +  mappingOneKPI(loaded_dict,timeFind, view, company, kpiList[0])
        else:
            return f'cụm chỉ tiêu "{groupKPI}" này chỉ có 1 chỉ tiêu con duy nhất là "{kpiList[0]}. "' + mappingOneKPI(loaeded_dict,timeFind, view, company, kpiList[0])
    ...
    for kpi in listIter:
        # print(kpi)
        command = mappingOneKPI(loaded_dict,timeFind, view, company, kpi,dict_index={'indexNow':indexMonthNow,
...
def mappingChildInferenceMom(loaded_dict,timeFind="",company="",kpi="",index=None):
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    if kpi == "":
        parentSet = loadParentSet(loaded_dict,company)
    ...
    if index == 1:
        if month == "1":
            ...
            listDate = generatePrevMonthList(timeFind)
            listKPIKoDat = []
            listKPIDat = []
            ...
            #==================can improve better==================
            res,eval = predictNextMonth(listKPIDat,listKPIKoDat)
...
def mappingCrossView(loaded_dict,timeFind,company="",kpi=""):
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    if kpi == "":
        parentSet = loadParentSet(loaded_dict,company)
...
def mappingViewStat(loaded_dict,timeFind,company="",kpi="",choice=""):
    if choice == "":
    ...
    groupTime = []
    if choice == 'season':
        timeList = genPreviousSeason(timeFind)
    else:
        timeList = generatePrevMonthList(timeFind)
...
def mappingViewPredict(loaded_dict,timeFind,company="",kpi="",choice=""):
    if choice == "":
        choice = random.choice(['season','monthBefore'])
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    ...
    resPredict = round(predictView(groupKPIVal),2)
    quarterDataset = loaded_dict[timeFind]['quarter'][company][kpi]
    yearDataset = loaded_dict[timeFind]['year'][company][kpi]
...
def mappingViewDetermineTrend(loaded_dict,timeFind,company="",kpi=""):
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    if kpi == "":
        parentSet = loadParentSet(loaded_dict,company)
    ...
    angle,res = determineTrend(groupKPIVal)
    indexLevel = 1
    indexDesc = 1
...
def mappingViewExplainResult(loaded_dict,timeFind,company="",kpi=""):
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    if kpi == "":
        parentSet = loadParentSet(loaded_dict,company)
...
def mappingViewOneKPIStat(loaded_dict,timeFind,view = "",company="",kpi="",choose=""):
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    if view == "":
    ...
    if choose == "mean": 
        ...
        monthFind =  int(timeList[index].split('/')[0])
    if monthFind != "":
        quarterFind = getQuarter(monthFind)
...
def mappingViewOneGroupKPIStat(loaded_dict,timeFind,view="",company="",groupKPI="",choose=""):
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    if view == "":
        view = random.choice(["month","month","month","quarter","year"])
...
def mappingCtyDescribe(loaded_dict,timeFind,view="",company=""):
    if company == "":
        company = random.choice(["VTS","VTT","VDS","VTPOST"])
    if view == "":
        view = random.choice(["month","quarter","year"])
...
#print(mappingGroupMonthDetail(loaded_dict,'07/2020',"month",'VTS','Tiến độ xử lý PAKH (theo từng dịch vụ trọng điểm)'))
